...
namespace puffinn {
    ...
    struct QueryMetrics {
        ...
        unsigned int hash_length_counts[32];
        unsigned int updates_per_hashlength[32];
    ...
    public:
        ...
        void add_hash_length(unsigned int hash_length) {
            if (PUFFINN_PERFORMANCE) {
                if (hash_length < 32) {
                    current_query().hash_length_counts[hash_length]++;
                }
            }
        }

        void add_updates_per_hashlength(unsigned int hash_length, uint64_t updates) {
            if (PUFFINN_PERFORMANCE) {
                if (hash_length < 32) {
                    current_query().updates_per_hashlength[hash_length] += updates;
                }
            }
        }
